...
#ifndef SRC_HYDROINFO_H5_H_
#define SRC_HYDROINFO_H5_H_
...
class HydroinfoH5 {
 ...
 public:
    ...
    HydroinfoH5(int XL_in, int XH_in, double DX_in, int LSX_in, int YL_in,
                int YH_in, double DY_in, int LSY_in, double Tau0_in,
                double dTau_in, double LST_in, int Visflag_in,
                string filename_in);

    ~HydroinfoH5();

    // functions to write into hdf5 file
    void setHydroFiles(int XL_in, int XH_in, double DX_in, int LSX_in,
    ...
    void addGroupattributeInt(hid_t H5groupEventid, string attName,
                              int attValue);
    void addGroupattributeDouble(hid_t H5groupEventid, string attName,
                                 double attValue);
    void writeHydroBlock(
        ...
        double **Pi11_in, double **Pi12_in, double **Pi13_in,
        double **Pi22_in, double **Pi23_in,
        double **Pi33_in, double ** BulkPi_in);
    void CSH5dumpBlockdata(hid_t group_id, const hsize_t * dims,
    ...
    int getNumberofFrames() {return(static_cast<int>(grid_Framenum));}
    ...
    void getHydroinfoOnlattice(int frameIdx, int xIdx, int yIdx,
