...
int DileptonQGPNLO::approx_rho(double *input, double &rT, double &rL) {
    ...
    double muoverBT = input[3];
    double muoverT = input[3]/3.;
    ...
    if (alphaS < al_min-1e-5) {
        ...
        double in[4] = {EoverT, koverT, al_min, muoverBT};
    ...
    if (MoverT > M_max+1e-5) {
        ...
        double in[4] = {EminOverT, koverT, alphaS, muoverBT};
        double min_rT, min_rL;
        approx_rho(in, min_rT, min_rL);
...
void DileptonQGPNLO::getRateFromTable(
    ...
    double in[4] = {E / T, k / T, alpha_s, muB / T };
