...
//----------------------------------------------------------------------
ParameterReader::~ParameterReader() {
    delete names;  // in principle garbage collection for vector should be
                   // automatic, just to be safe
    delete values;
}
...
/*
  Remove comments from a string "str". Comments are all characters after the
...
/*
  Phrase an equation like "x=1", and store the result into "names" and "values".
  The equation is first separated according to the equal sign, then the left and
  right hand side will be trimmed, after that the right hand side will be
  converted to double type number.
*/
{
...
/*
  Check if the parameter with "name" already exists in the internal "names"
  list. If yes, it returns its
*/
{
...
/*
  Read all lines in a file as parameter assignment list. Each line is processed
  by the phraseOneLine function.
*/
{
    ...
    while (!parameterFile.eof()) {
        ...
        phraseOneLine(buffer, commentSymbol);
        // echo(); // for debug
    }
    parameterFile.close();
}
...
/*
  Set the parameter with "name" to "value". It is appended to the internal
  "names" and "values" vector if "name" does not exist; otherwise it is
  rewitten.
*/
{
    long idx = find(name);
...
double ParameterReader::getVal(string name, double value) {
    ...
    if (idx != -1) {
        ...
        cout << "ParameterReader::getVal error: parameter with name " << name
             << " not found. Using the default value: " << value << endl;
        return (value);
    }
    return (value);
}
