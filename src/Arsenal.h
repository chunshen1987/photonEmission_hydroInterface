...
double interpCubicDirect(
    std::vector<double>* x, std::vector<double>* y, double xx);
double interpCubicMono(
    std::vector<double>* x, std::vector<double>* y, double xx);
double interpLinearDirect(
    std::vector<double>* x, std::vector<double>* y, double xx);
double interpLinearMono(
    std::vector<double>* x, std::vector<double>* y, double xx);
double interpNearestDirect(
    std::vector<double>* x, std::vector<double>* y, double xx);
double interpNearestMono(
    std::vector<double>* x, std::vector<double>* y, double xx);

double invertFunc(
    double (*func)(double), double y, double xL, double xR, double dx,
    double x0, double relative_accuracy = 1e-10);

double invertTableDirect_hook(double xx);
double invertTableDirect(
    std::vector<double>* x, std::vector<double>* y, double y0, double x0,
    double relative_accuracy = 1e-10);

long binarySearch(std::vector<double>* A, double value);

void outputFunctionerror(
    std::string function_name, std::string massage, double value, int level);
