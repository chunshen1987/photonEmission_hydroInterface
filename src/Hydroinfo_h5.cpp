...
HydroinfoH5::HydroinfoH5(string filename_in, int bufferSize_in,
                         int Visflag_in) {
...
HydroinfoH5::HydroinfoH5(int XL_in, int XH_in, double DX_in, int LSX_in,
    ...
    setHydroFiles(XL_in, XH_in, DX_in, LSX_in,
...
void HydroinfoH5::setHydroFiles(int XL_in, int XH_in, double DX_in, int LSX_in,
                                int YL_in, int YH_in, double DY_in, int LSY_in,
                                double Tau0_in, double dTau_in, double LST_in,
                                int Visflag_in, string filename_in) {
    outputFlag = 1;
    ...
    dimensionX = static_cast<int>((grid_XH - grid_XL - 2*XShift)/grid_LSX) + 1;
    dimensionY = static_cast<int>((grid_YH - grid_YL - 2*YShift)/grid_LSY) + 1;

    filename = filename_in;
    herr_t status;
    /* Create a new file using default properties. */
    H5file_id = H5Fcreate(filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                          H5P_DEFAULT);

    /* Create a group named "/Event" in the file. */
    H5groupEventid = H5Gcreate(H5file_id, "/Event", H5P_DEFAULT, H5P_DEFAULT,
                               H5P_DEFAULT);
...
void HydroinfoH5::readHydroinfoH5(string filename_in, int bufferSize_in,
                                  int Visflag_in) {
    readinFlag = 1;
...
void HydroinfoH5::readHydrogridInfo() {
    ...
    grid_Framenum = static_cast<int>(tempFramenum);
...
void HydroinfoH5::readHydroinfoBuffered_total() {
    ...
    for (int i = 0; i < Buffersize; i++) {
        ...
        if (frameIdx < static_cast<int>(grid_Framenum)) {
            ...
            group_id = H5Gopen(H5groupEventid, frameName.str().c_str(),
                               H5P_DEFAULT);
...
void HydroinfoH5::readHydroinfoSingleframe(int frameIdx) {
    ...
    if (frameIdx < static_cast<int>(grid_Framenum)) {
        ...
        cout << "Error: readHydroinfoSingleframe :: "
             << "frameIdx exceed maximum frame number from hydro" << endl;
        cout << "frameIdx = " << frameIdx << endl;
        exit(1);
    }
}

void HydroinfoH5::readH5Dataset_double(hid_t id, string datasetName,
                                       double** dset_data) {
    herr_t status;
    hid_t dataset_id;
...
void HydroinfoH5::getHydroinfoOnlattice(int frameIdx, int xIdx, int yIdx,
                                        fluidCell* fluidCellptr) {
    if (frameIdx < 0 || frameIdx > grid_Framenum
        || xIdx < 0 || xIdx > (grid_XH - grid_XL)
        || yIdx < 0 || yIdx > (grid_YH - grid_YL)) {
...
void HydroinfoH5::getHydroinfo(double tau, double x, double y,
                               fluidCell* fluidCellptr) {
    double eps = 1e-10;
    if (tau < grid_Tau0 || tau > grid_Taumax-eps
        || x < grid_X0 || x > grid_Xmax-eps
        || y < grid_Y0 || y > grid_Ymax-eps) {
        setZero_fluidCell(fluidCellptr);
        return;
    }
    double temp;
    temp = (tau - grid_Tau0)/grid_dTau;
    int frameIdx = static_cast<int>(temp);
    double tauInc = temp - frameIdx;

    temp = (x - grid_X0)/grid_dx;
    int xIdx = static_cast<int>(temp);
    double xInc = temp - xIdx;

    temp = (y - grid_Y0)/grid_dy;
    int yIdx = static_cast<int>(temp);
    double yInc = temp - yIdx;

    fluidCellptr->ed = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, ed);
    fluidCellptr->sd = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, sd);
    fluidCellptr->vx = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, vx);
    fluidCellptr->vy = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, vy);
    fluidCellptr->temperature = cubeInterpShell(
                        xIdx, yIdx, frameIdx, xInc, yInc, tauInc, Temperature);
    fluidCellptr->pressure = cubeInterpShell(
                        xIdx, yIdx, frameIdx, xInc, yInc, tauInc, Pressure);
    if (Visflag == 1) {
        fluidCellptr->pi[0][0] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi00);
        fluidCellptr->pi[0][1] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi01);
        fluidCellptr->pi[0][2] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi02);
        fluidCellptr->pi[0][3] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi03);
        fluidCellptr->pi[1][0] = fluidCellptr->pi[0][1];
        fluidCellptr->pi[1][1] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi11);
        fluidCellptr->pi[1][2] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi12);
        fluidCellptr->pi[1][3] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi13);
        fluidCellptr->pi[2][0] = fluidCellptr->pi[0][2];
        fluidCellptr->pi[2][1] = fluidCellptr->pi[1][2];
        fluidCellptr->pi[2][2] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi22);
        fluidCellptr->pi[2][3] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi23);
        fluidCellptr->pi[3][0] = fluidCellptr->pi[0][3];
        fluidCellptr->pi[3][1] = fluidCellptr->pi[1][3];
        fluidCellptr->pi[3][2] = fluidCellptr->pi[2][3];
        fluidCellptr->pi[3][3] = cubeInterpShell(
                                xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi33);
        fluidCellptr->bulkPi = cubeInterpShell(
                            xIdx, yIdx, frameIdx, xInc, yInc, tauInc, BulkPi);
    } else {
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
...
double HydroinfoH5::cubeInterpShell(int idx_x, int idx_y, int idx_z,
                                    double x, double y, double z,
                                    double ***dataset) {
    double result = cubeInterp(x, y, z,
        dataset[idx_z][idx_x][idx_y], dataset[idx_z][idx_x+1][idx_y],
        dataset[idx_z][idx_x][idx_y+1], dataset[idx_z][idx_x+1][idx_y+1],
        dataset[idx_z+1][idx_x][idx_y], dataset[idx_z+1][idx_x+1][idx_y],
        dataset[idx_z+1][idx_x][idx_y+1], dataset[idx_z+1][idx_x+1][idx_y+1]);
    return(result);
}

double HydroinfoH5::cubeInterp(double x, double y, double z,
                               double A000, double A100, double A010,
                               double A110, double A001, double A101,
                               double A011, double A111) {
