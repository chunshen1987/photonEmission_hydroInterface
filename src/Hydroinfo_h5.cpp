...
HydroinfoH5::HydroinfoH5(
    string filename_in, int bufferSize_in, int Visflag_in) {
    readHydroinfoH5(filename_in, bufferSize_in, Visflag_in);
}

HydroinfoH5::HydroinfoH5(
    int XL_in, int XH_in, double DX_in, int LSX_in, int YL_in, int YH_in,
    double DY_in, int LSY_in, double Tau0_in, double dTau_in, double LST_in,
    int Visflag_in, string filename_in) {
    setHydroFiles(
        XL_in, XH_in, DX_in, LSX_in, YL_in, YH_in, DY_in, LSY_in, Tau0_in,
        dTau_in, LST_in, Visflag_in, filename_in);
}

HydroinfoH5::~HydroinfoH5() {
    if (readinFlag == 1) {
        for (int i = 0; i < Buffersize; i++) {
            for (unsigned int j = 0; j < dimensionX; j++) {
                delete[] ed[i][j];
                delete[] sd[i][j];
                delete[] vx[i][j];
                delete[] vy[i][j];
...
void HydroinfoH5::setHydroFiles(
    int XL_in, int XH_in, double DX_in, int LSX_in, int YL_in, int YH_in,
    double DY_in, int LSY_in, double Tau0_in, double dTau_in, double LST_in,
    int Visflag_in, string filename_in) {
    outputFlag = 1;
    ...
    H5groupEventid =
        H5Gcreate(H5file_id, "/Event", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);

    writeGroupattribute(H5groupEventid);

    /* Close the group. */
    H5Gclose(H5groupEventid);
...
void HydroinfoH5::addGroupattributeInt(
    hid_t H5groupEventid, string attName, int attValue) {
    hsize_t dims;
    ...
    attribute_id = H5Acreate(
        H5groupEventid, attName.c_str(), H5T_STD_I32BE, dataspace_id,
        H5P_DEFAULT, H5P_DEFAULT);

    /* Write the attribute data. */
    H5Awrite(attribute_id, H5T_NATIVE_INT, &attValue);
...
void HydroinfoH5::addGroupattributeDouble(
    hid_t H5groupEventid, string attName, double attValue) {
    hsize_t dims;
    hid_t attribute_id, dataspace_id;
    ...
    /* Create a dataset attribute. */
    attribute_id = H5Acreate(
        H5groupEventid, attName.c_str(), H5T_NATIVE_DOUBLE, dataspace_id,
        H5P_DEFAULT, H5P_DEFAULT);

    /* Write the attribute data. */
    H5Awrite(attribute_id, H5T_NATIVE_DOUBLE, &attValue);
...
void HydroinfoH5::writeHydroBlock(
    int Time_id, double** ed_in, double** sd_in, double** p_in,
    double** Temp_in, double** Vx_in, double** Vy_in, double** Pi00_in,
    double** Pi01_in, double** Pi02_in, double** Pi03_in, double** Pi11_in,
    double** Pi12_in, double** Pi13_in, double** Pi22_in, double** Pi23_in,
    double** Pi33_in, double** BulkPi_in) {
    if (LST_cur != 0) {
        LST_cur--;
        return;
    } else {
    ...
    groupFrameid = H5Gcreate(
        H5groupEventid, frameName.str().c_str(), H5P_DEFAULT, H5P_DEFAULT,
        H5P_DEFAULT);
    addGroupattributeDouble(
        groupFrameid, "Time", grid_Tau0 + grid_dTau * grid_LST * Frame_id);
...
void HydroinfoH5::CSH5dumpBlockdata(
    hid_t group_id, const hsize_t* dims, string DatasetName, double** Dataset) {
    ...
    dataset_id = H5Dcreate(
        group_id, DatasetName.c_str(), H5T_NATIVE_DOUBLE, dataspace_id,
...
void HydroinfoH5::readHydroinfoH5(
    string filename_in, int bufferSize_in, int Visflag_in) {
    readinFlag = 1;
    ...
    if (Buffersize < grid_Framenum) {
        cout << "Buffersize is too small, increase it to at lease to "
             << grid_Framenum << endl;
        exit(1);
    }
...
void HydroinfoH5::readHydrogridInfo() {
    ...
    H5Gget_num_objs(H5groupEventid, &tempFramenum);
...
int HydroinfoH5::readH5Attribute_int(hid_t id, string attributeName) {
    ...
    H5Aread(attr, H5T_NATIVE_INT, &attributeValue);
    H5Aclose(attr);
...
double HydroinfoH5::readH5Attribute_double(hid_t id, string attributeName) {
    ...
    hid_t attr;
    attr = H5Aopen_name(id, attributeName.c_str());
    H5Aread(attr, H5T_NATIVE_DOUBLE, &attributeValue);
    H5Aclose(attr);
    return (attributeValue);
}
...
void HydroinfoH5::readHydroinfoBuffered_total() {
    ...
    for (int i = 0; i < Buffersize; i++) {
        frameIdx = i;
        if (frameIdx < static_cast<int>(grid_Framenum)) {
            stringstream frameName;
            frameName << "Frame_" << setw(4) << setfill('0') << frameIdx;
            group_id =
                H5Gopen(H5groupEventid, frameName.str().c_str(), H5P_DEFAULT);
...
void HydroinfoH5::readHydroinfoSingleframe(int frameIdx) {
    ...
    if (frameIdx < (int)grid_Framenum) {
        ...
        cout << "Error: readHydroinfoSingleframe :: frameIdx exceed maximum "
                "frame number from hydro"
             << endl;
...
void HydroinfoH5::readH5Dataset_double(
    hid_t id, string datasetName, double** dset_data) {
    hid_t dataset_id;
...
void HydroinfoH5::getHydroinfoOnlattice(
    int frameIdx, int xIdx, int yIdx, fluidCell* fluidCellptr) {
    if (frameIdx < 0 || frameIdx > grid_Framenum || xIdx < 0
        || xIdx > (grid_XH - grid_XL) || yIdx < 0
        || yIdx > (grid_YH - grid_YL)) {
...
void HydroinfoH5::getHydroinfo(
    double tau, double x, double y, fluidCell* fluidCellptr) {
    double eps = 1e-10;
    if (tau < grid_Tau0 || tau > grid_Taumax - eps || x < grid_X0
        || x > grid_Xmax - eps || y < grid_Y0 || y > grid_Ymax - eps) {
    ...
    fluidCellptr->ed =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, ed);
    fluidCellptr->sd =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, sd);
    fluidCellptr->vx =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, vx);
    fluidCellptr->vy =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, vy);
    fluidCellptr->temperature =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, Temperature);
    fluidCellptr->pressure =
        cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, Pressure);
    if (Visflag == 1) {
        fluidCellptr->pi[0][0] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi00);
        fluidCellptr->pi[0][1] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi01);
        fluidCellptr->pi[0][2] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi02);
        fluidCellptr->pi[0][3] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi03);
        fluidCellptr->pi[1][0] = fluidCellptr->pi[0][1];
        fluidCellptr->pi[1][1] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi11);
        fluidCellptr->pi[1][2] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi12);
        fluidCellptr->pi[1][3] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi13);
        fluidCellptr->pi[2][0] = fluidCellptr->pi[0][2];
        fluidCellptr->pi[2][1] = fluidCellptr->pi[1][2];
        fluidCellptr->pi[2][2] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi22);
        fluidCellptr->pi[2][3] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi23);
        fluidCellptr->pi[3][0] = fluidCellptr->pi[0][3];
        fluidCellptr->pi[3][1] = fluidCellptr->pi[1][3];
        fluidCellptr->pi[3][2] = fluidCellptr->pi[2][3];
        fluidCellptr->pi[3][3] =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, pi33);
        fluidCellptr->bulkPi =
            cubeInterpShell(xIdx, yIdx, frameIdx, xInc, yInc, tauInc, BulkPi);
    } else {
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++) fluidCellptr->pi[i][j] = 0.0e0;
...
double HydroinfoH5::cubeInterpShell(
    int idx_x, int idx_y, int idx_z, double x, double y, double z,
    double*** dataset) {
    double result = cubeInterp(
        x, y, z, dataset[idx_z][idx_x][idx_y], dataset[idx_z][idx_x + 1][idx_y],
        dataset[idx_z][idx_x][idx_y + 1], dataset[idx_z][idx_x + 1][idx_y + 1],
        dataset[idx_z + 1][idx_x][idx_y], dataset[idx_z + 1][idx_x + 1][idx_y],
        dataset[idx_z + 1][idx_x][idx_y + 1],
        dataset[idx_z + 1][idx_x + 1][idx_y + 1]);
    return (result);
}

double HydroinfoH5::cubeInterp(
...
// constrained to the unit cube. Axyz is the return value.
{
    ...
    double Axyz = A000 * (1 - x) * (1 - y) * (1 - z)
                  + A100 * x * (1 - y) * (1 - z) + A010 * (1 - x) * y * (1 - z)
                  + A001 * (1 - x) * (1 - y) * z + A101 * x * (1 - y) * z
